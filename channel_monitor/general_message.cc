...
int GeneralMessage::Render(const Screen* s, int key) {
  ...
  int line_no = 0;
  {
    ...
    int length = 0;
    auto msg_buffer = channelMsgPtr->CopyBufferPtr(length);
    if (!channelMsgPtr->raw_msg_class_->ParseFromArray(*msg_buffer, length)) {
      // if (!channelMsgPtr->raw_msg_class_->ParseFromString(*channelMsg)) {
      s->AddStr(0, line_no++,
